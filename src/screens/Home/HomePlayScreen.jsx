...
import { useSendParamsMutation, useUnitsGetDayTimersQuery, useGetTimersUnitQuery, useGetParamsQuery } from '../../redux/usersApi';
...
function HomePlayScreen({ navigation, route }) {
  ...
  const { data: timers, isLoading: isLoadingTimers } = useGetTimersUnitQuery({ controllerId: unitId, day: currentDayOfWeek });

  const { data: unitParams, error: errorunitParamsDevices, isLoading: isLoadingUnitParams, refetch: refetchUnitParams } = useGetParamsQuery({ controllerId: unitId });
  ...
  useEffect(() => {
    ...
    //   const resDataStorage = await AsyncStorage.getItem("res");
    //   if (resDataStorage) {
    //     handleDisabled(resDataStorage);
    //     setResMode(resDataStorage);
    //   }
    // };
    ...
    return (
      ...
      >
        <LinearGradient
          ...
          style={{ borderRadius: 8, width: 40, height: 40, marginBottom: 5, alignItems: 'center', justifyContent: 'center', }}
        >
        ...
        {(item[0] === "tempTarget") && <><Text style={styles.boxPowerBtnTextName}>Температура</Text>
          <Text style={styles.boxPowerBtnText}>{temperature}°</Text></>}

        {(item[0] === "humRoomTarget") && <><Text style={styles.boxPowerBtnTextName}>Влажность</Text>
          <Text style={styles.boxPowerBtnText}>{humTarget}%</Text></>}

        {(item[0] === "fanSpeedP") && <><Text style={styles.boxPowerBtnTextName}>Скорость</Text>
          <Text style={styles.boxPowerBtnText}>{fanTarget}</Text></>}

        {(item[0] === "res") && <><Text style={styles.boxPowerBtnTextName}>Режим</Text>
          <Text style={styles.boxPowerBtnText}>{(resMode != '0') ? resNames[resMode] : 'Не выбрано'}</Text></>}

        {(item[0] === "ZagrFiltr") && <><Text style={styles.boxPowerBtnTextName}>Автозапуск</Text>
  ...
  const sendParamsHum = async () => {
    ...
    // console.log('sendParamsHum', data);
    try {
  ...
  const sendParamsRes = async () => {
    ...
    try {
      const answer = await sendParams(resData)
      console.log('answerSendParamsRes', answer.data.data);
      refetchUnitParams();
    ...
    <SafeAreaView style={{ flex: 1 }}>
      <ScrollView>
        ...
        <ScheduleModal
          ...
          setSpeedSuccess={setSpeedSuccess}
        ...
        <HumidityModal
          ...
          sendParamsHum={sendParamsHum}
          setHumTarget={setHumTarget}
          humTarget={humTarget}
          unitParams={unitParams}
        />
        ...
        <ModeModal
          ...
          sendParamsRes={sendParamsRes}
          unitParams={unitParams}
          resMode={resMode}
          setResMode={setResMode}
        />

        {modalVisibleSpeed && <SpeedModal
          setModalVisibleSpeed={setModalVisibleSpeed}
          modalVisibleSpeed={modalVisibleSpeed}
          unitsId={unitId}
          unitParams={unitParams}
          setFanTarget={setFanTarget}
          fanTarget={fanTarget}
          setSpeedSuccess={setSpeedSuccess}
          setSpeedError={setSpeedError}
          setErrorText={setErrorText}
          setSuccessText={setSuccessText}
          refetchUnitParams={refetchUnitParams}
        ...
        <View style={styles.container}>
          ...
          >
            ...
            {unitParams && <View style={styles.boxHomeDeviceFunctions}>
              {Object.entries(unitParams?.data[0]).map((item, index) => {
                ...
                return (
                  <View style={styles.boxHomeDeviceFunctionsItem} key={index}>
                    ...
                    {(item[0] == 'humRoom') && <Text style={styles.boxDeviceFunctionsItemText}>{Math.round(item[1])}%</Text>}

                    {(item[0] == 'tempChannel') && <Text style={styles.boxDeviceFunctionsItemName}>Температура на улице</Text>}
                    {(item[0] == 'tempChannel') && <Text style={styles.boxDeviceFunctionsItemText}>{Math.round(item[1])}°C</Text>}

                    {(item[0] == 'tempRoom') && <Text style={styles.boxDeviceFunctionsItemName}>Температура в помещении</Text>}
                    {(item[0] == 'tempRoom') && <Text style={styles.boxDeviceFunctionsItemText}>{Math.round(item[1])}°C</Text>}

                    {(item[0] == 'fanSpeedP') && <Text style={styles.boxDeviceFunctionsItemName}>Скорость вращения</Text>}
                    {(item[0] == 'fanSpeedP') && <Text style={styles.boxDeviceFunctionsItemText}>{Math.round(item[1])}</Text>}
