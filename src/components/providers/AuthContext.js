...
export const AuthProvider = ({children}) => {
    ...
    const signOut = async () => {
        try {
            ...
            setIsLoading(false);
            setUserName(null)
            console.log('токен удален, UserId удален,  UserName удален, UnitId удален,controllers удален, email удален, SecureStore очищен');
        } catch (error) {
    ...
    return (
        <AuthContext.Provider value={{userToken, isLoading, signIn, signOut, unitId, userId, allControllers, emailAuthContext, userName}}>
            {children}
